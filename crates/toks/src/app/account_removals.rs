...
use toks_core::accounts::{AccountOrderKey, ProviderAccount};
...
pub(crate) fn request_removal(app: &mut ToksApp, key: AccountOrderKey, cx: &mut Context<ToksApp>) {
    ...
    cx.spawn(async move |this, cx| {
        ...
        let result = cx
            .background_spawn(
                async move { toks_core::accounts::remove_from_toks(provider, &account) },
            )
            .await;
