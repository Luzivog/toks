...
impl AccountOperations {
    pub(crate) fn authenticated_accounts(&self) -> Vec<AccountOrderKey> {
        self.pending
            ...
            .filter(|pending| {
                matches!(
                    toks_core::accounts::login_outcome(
                        pending.key.provider,
                        &pending.key.profile_id,
                    ),
                    Some(
                        toks_core::accounts::LoginOutcome::Authenticated
                            | toks_core::accounts::LoginOutcome::IdentityChanged
                    )
                )
            })
            .filter_map(|pending| {
                toks_core::accounts::unhide_profile(pending.key.provider, &pending.key.profile_id)
                    .ok()
                    .flatten()
                    .map(|account_id| {
                        AccountOrderKey::new(pending.key.provider, account_id.as_str())
                    })
            })
            .collect()
    }
    ...
    pub(crate) fn start_add(
        ...
        if !limits.iter().any(|snapshot| {
            ...
            toks_core::accounts::apply_saved_order(limits);
        }
