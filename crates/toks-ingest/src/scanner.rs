...
/// like `TOKS_EXTRA_DIRS` — it lives on the `scanner` key inside
/// `~/.config/toks/settings.json` and is threaded down into
...
pub struct ScannerSettings {
    ...
    /// data directory, so Toks' auto-discovery can't find it.
    ///
...
pub fn headless_roots_with_env_strategy(home_dir: &str, use_env_roots: bool) -> Vec<PathBuf> {
    ...
    roots.push(crate::paths::resolved_named_root(&config_parent).join("headless"));

    let application_support = PathBuf::from(join_native(home_dir, "Library/Application Support"));
    roots.push(crate::paths::resolved_named_root(&application_support).join("headless"));
...
/// settings (e.g. from `~/.config/toks/settings.json`). Thin wrappers
...
fn scan_all_clients_with_env_strategy_inner(
    ...
    if use_env_roots {
        let extra_dirs_val =
            crate::paths::renamed_env_var("TOKS_EXTRA_DIRS", "TOKSCOPE_EXTRA_DIRS")
                .unwrap_or_default();
