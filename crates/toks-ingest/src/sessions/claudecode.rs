...
use super::utils::{
    estimate_tokens, extract_i64, extract_string, file_modified_timestamp_ms, lossy_lines,
...
pub fn parse_claude_file_with_cache_and_home(
    ...
    // counts the same content (tokscope#1011). Bare transcripts under
...
struct ClaudeToolResultContext<'a> {
    ...
    /// tool_result text, so the char fallback double-counts (tokscope#1011).
