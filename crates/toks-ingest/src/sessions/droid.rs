...
use super::utils::{
    file_modified_timestamp_ms, lossy_lines, provider_from_model_or, read_file_or_none,
};
...
pub fn parse_droid_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    let provider = settings.provider_lock.clone().unwrap_or_else(|| {
        provider_from_model_or(settings.model.as_deref().unwrap_or(""), "unknown").to_string()
    });
