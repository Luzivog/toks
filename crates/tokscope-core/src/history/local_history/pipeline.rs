...
use chrono::Utc;
use tokscope_ingest::accounting_delta::{AccountingDeltaCollector, AccountingDeltaOptions};
...
pub(super) fn hydrate_archive() -> Result<Option<RefreshBatch>> {
    ...
    let pending_sources = projection
        .pending_sources
        .saturating_add(projection_backlog(&projection));
    let settings = tokscope_ingest::scanner::ScannerSettings::default();
...
fn migrate_projection_before_ingest(
    settings: &tokscope_ingest::scanner::ScannerSettings,
...
fn scanner_settings() -> tokscope_ingest::scanner::ScannerSettings {
    ...
    tokscope_ingest::scanner::ScannerSettings {
