//! Cross-platform resolution for tokscope's user config and cache dirs.
//!
//! Tokscope-core needs the same path helpers tokscope-cli uses (settings
//! and message/pricing caches read from related directories), so the
//! resolver lives here and is re-exported from tokscope-cli for callers
//! that already imported it from there. macOS users following the docs
//! expect `~/.config/tokscope/` because that is what `auth.rs`,
...
//! enforces the unified `~/.config/tokscope/` location on macOS + Linux,
...
/// Resolve the tokscope config dir, honoring `TOKSCOPE_CONFIG_DIR` first.
///
/// Resolution order:
/// 1. `TOKSCOPE_CONFIG_DIR` taken verbatim when set to a non-empty value.
...
/// 2. macOS: `$HOME/.config/tokscope` (overrides `dirs::config_dir()`,
///    which would return `~/Library/Application Support/` and split state
///    across two roots — see module docs).
/// 3. Linux: `dirs::config_dir().join("tokscope")` so XDG_CONFIG_HOME is
///    honored. Falls through to `$HOME/.config/tokscope` when neither
///    `XDG_CONFIG_HOME` nor `HOME` resolve.
/// 4. Windows (and any other platform): `dirs::config_dir().join("tokscope")`.
/// 5. Last-ditch fallback: `./.tokscope` so a missing HOME never panics.
pub fn get_config_dir() -> PathBuf {
    if let Some(custom) = std::env::var_os("TOKSCOPE_CONFIG_DIR") {
        if !custom.is_empty() {
    ...
    #[cfg(target_os = "macos")]
    {
        if let Some(home) = home_dir() {
            return home.join(".config").join("tokscope");
        }
    }

    dirs::config_dir()
        .map(|d| d.join("tokscope"))
        .unwrap_or_else(|| PathBuf::from(".tokscope"))
}

/// Resolve the tokscope cache dir as `<config_dir>/cache`.
///
...
/// single subdirectory so an isolated profile (`TOKSCOPE_CONFIG_DIR=...`)
...
/// Whether `TOKSCOPE_CONFIG_DIR` is explicitly set in the environment.
///
...
pub fn is_config_dir_overridden() -> bool {
    std::env::var_os("TOKSCOPE_CONFIG_DIR").is_some_and(|v| !v.is_empty())
}
...
/// in this crate that redirect `HOME` or `TOKSCOPE_CONFIG_DIR`.
///
...
mod tests {
    ...
    fn guard() -> EnvGuard {
        EnvGuard::capture(&["TOKSCOPE_CONFIG_DIR", "HOME", "XDG_CONFIG_HOME"])
    }
    ...
    fn unix_default_is_dot_config_tokscope_under_home() {
        let mut env = guard();
        env.remove("TOKSCOPE_CONFIG_DIR");
        env.remove("XDG_CONFIG_HOME");
        env.set("HOME", "/tmp/tokscope-core-paths-home");
        assert_eq!(
            get_config_dir(),
            PathBuf::from("/tmp/tokscope-core-paths-home/.config/tokscope"),
        );
    }
    ...
    fn linux_honors_xdg_config_home_when_set() {
        let mut env = guard();
        env.remove("TOKSCOPE_CONFIG_DIR");
        env.set("XDG_CONFIG_HOME", "/tmp/tokscope-core-paths-xdg");
        assert_eq!(
            get_config_dir(),
            PathBuf::from("/tmp/tokscope-core-paths-xdg/tokscope"),
        );
    }
    ...
    fn get_config_dir_treats_empty_override_as_unset() {
        ...
        assert!(
            resolved.is_absolute() || resolved == Path::new(".tokscope"),
