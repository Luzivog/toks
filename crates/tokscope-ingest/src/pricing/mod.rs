...
impl PricingService {
    ...
    fn degrade_source(
        ...
        match result {
            ...
            Err(error) => {
                let cached = cached();
                eprintln!(
                    "[tokscope] Warning: {} pricing fetch failed ({}); {}",
                    label,
                    error,
                    if cached.is_some() {
    ...
    fn start_background_refresh(service: Arc<Self>) {
        let cache_only = std::env::var("TOKSCOPE_PRICING_CACHE_ONLY")
        ...
        let spawn = std::thread::Builder::new()
            .name("tokscope-pricing-refresh".into())
            .spawn(move || {
                    ...
                    .build()
                {
                    Ok(runtime) => runtime,
                    Err(error) => {
                        eprintln!("[tokscope] Warning: pricing refresh could not start: {error}");
                        return;
                    }
                };
                match runtime.block_on(Self::fetch_inner()) {
                    ...
                    Err(error) => {
                        eprintln!("[tokscope] Warning: pricing refresh failed: {error}")
                    }
                }
            });
        if let Err(error) = spawn {
            REFRESH_STARTED.store(false, Ordering::Release);
            eprintln!("[tokscope] Warning: pricing refresh thread failed: {error}");
        }
    }
