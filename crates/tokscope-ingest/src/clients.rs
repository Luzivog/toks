            ...
            PathRoot::Config => {
                if use_env_roots {
                    ...
                    if let Ok(xdg_config_home) = std::env::var("XDG_CONFIG_HOME") {
                        return format!("{xdg_config_home}/tokscope");
                    }
                    ...
                    if let Some(dir) = dirs::config_dir() {
                        return dir.join("tokscope").to_string_lossy().into_owned();
                    }
                }
                ...
                if !use_env_roots {
                    return std::path::Path::new(home_dir)
                        .join("AppData/Roaming/tokscope")
                        .to_string_lossy()
                        .into_owned();
                }

                join_home(home_dir, ".config/tokscope")
            }
            PathRoot::EnvVar {
                var,
                fallback_relative,
...
define_clients!(
    ...
    Cursor = 3 => {
        id: "cursor",
        display: "Cursor IDE",
        logo: Some("https://tokscope.ai/assets/logos/cursor.jpg"),root: PathRoot::Home,
        relative: ".config/tokscope/cursor-cache",
        pattern: "usage*.csv",
        headless: false,
        parse_local: false,
        submit_default: true
...
mod tests {
    ...
    fn test_reasonix_stats_expands_environment_references_before_normalizing_paths() {
        let mut env = EnvGuard::capture(&[
            "REASONIX_STATE_HOME",
            "TOKSCOPE_REASONIX_TEST_ROOT",
            "TOKSCOPE_REASONIX_TEST_UNSET",
        ]);
        let client = ClientId::Reasonix;

        env.set("TOKSCOPE_REASONIX_TEST_ROOT", "~/reasonix-state");
        env.remove("TOKSCOPE_REASONIX_TEST_UNSET");
        env.set(
            "REASONIX_STATE_HOME",
            "${TOKSCOPE_REASONIX_TEST_ROOT}/nested",
        );
        assert_eq!(
            client.data().resolve_path(reasonix_home()),
            reasonix_stats_under(native_join(
        ...
        env.set(
            "REASONIX_STATE_HOME",
            "${TOKSCOPE_REASONIX_TEST_UNSET:-relative-reasonix}",
        );
        let expected = reasonix_stats_under(
            std::env::current_dir()
                .expect("test process has a current directory")
    ...
    fn test_path_root_config_uses_override_when_set() {
        let previous_override = std::env::var("TOKSCOPE_CONFIG_DIR").ok();
        let previous_xdg = std::env::var("XDG_CONFIG_HOME").ok();
        unsafe {
            std::env::set_var("TOKSCOPE_CONFIG_DIR", "/tmp/custom-config-root");
        ...
        restore_env("TOKSCOPE_CONFIG_DIR", previous_override);
    ...
    fn test_path_root_config_uses_xdg_config_home_when_override_unset() {
        ...
        let previous_xdg = std::env::var("XDG_CONFIG_HOME").ok();
        unsafe {
            std::env::remove_var("TOKSCOPE_CONFIG_DIR");
            std::env::set_var("XDG_CONFIG_HOME", "/tmp/xdg-config-home");
        }

        let resolved = PathRoot::Config.resolve("/tmp/home");
        assert_eq!(resolved, "/tmp/xdg-config-home/tokscope");
    ...
    fn test_path_root_config_uses_dirs_config_dir_on_windows() {
        ...
        // i.e. dirs::config_dir() (= %APPDATA%\tokscope). Hardcoding
        // {home}/.config/tokscope would diverge from the writer side
        ...
        assert_eq!(
            resolved, expected,
            "PathRoot::Config on Windows must match dirs::config_dir().join('tokscope') so the scanner agrees with the writer"
        );
    ...
    fn test_path_root_config_ignores_env_when_disabled() {
        ...
        let expected = if cfg!(target_os = "windows") {
            ...
            native_join(std::path::Path::new("/tmp/home"), ".config/tokscope")
        };
    ...
    fn test_path_root_env_var_uses_env_when_set() {
        let var = "TOKSCOPE_TEST_PATH_ROOT";
