...
/// like `TOKSCOPE_EXTRA_DIRS` — it lives on the `scanner` key inside
/// `~/.config/tokscope/settings.json` and is threaded down into
...
pub struct ScannerSettings {
    ...
    /// data directory, so tokscope's auto-discovery can't find it.
    ///
    ...
    /// `tui::settings::pin_bucket_timezone_if_unset`); `tokscope config set
...
pub fn headless_roots_with_env_strategy(home_dir: &str, use_env_roots: bool) -> Vec<PathBuf> {
    if use_env_roots {
        if let Ok(path) = std::env::var("TOKSCOPE_HEADLESS_DIR") {
    ...
    roots.push(PathBuf::from(join_native(
        home_dir,
        ".config/tokscope/headless",
    )));

    let mac_root = PathBuf::from(join_native(
        home_dir,
        "Library/Application Support/tokscope/headless",
    ));
    roots.push(mac_root);
...
/// Parse a `TOKSCOPE_EXTRA_DIRS`-formatted string into (ClientId, path) pairs.
///
...
/// `tokscope submit`) sibling/default profiles from a profile-scoped
...
/// settings (e.g. from `~/.config/tokscope/settings.json`). Thin wrappers
...
fn scan_all_clients_with_env_strategy_inner(
    ...
    if use_env_roots {
        let extra_dirs_val = std::env::var("TOKSCOPE_EXTRA_DIRS").unwrap_or_default();
    ...
    if enabled.contains(&ClientId::OpenCode) {
        ...
        // `tokscope --claude` does not pull in OpenCode dbs the user pinned
...
mod tests {
    ...
    fn test_env_guard_restores_after_unwind() {
        const KEY: &str = "TOKSCOPE_SCANNER_ENV_GUARD_SELF_CHECK";
    ...
    fn test_headless_roots_default() {
        let previous = std::env::var("TOKSCOPE_HEADLESS_DIR").ok();
        unsafe { std::env::remove_var("TOKSCOPE_HEADLESS_DIR") };

        let home = "/tmp/tokscope-test-home";
        let roots = headless_roots(home);
        let config_root = PathBuf::from(join_native(home, ".config/tokscope/headless"));
        ...
        restore_env("TOKSCOPE_HEADLESS_DIR", previous);
    }
    ...
    fn test_headless_roots_override() {
        let previous = std::env::var("TOKSCOPE_HEADLESS_DIR").ok();
        unsafe { std::env::set_var("TOKSCOPE_HEADLESS_DIR", "/custom/headless") };
    ...
    fn test_headless_roots_ignore_env_override_when_disabled() {
        ...
        assert_eq!(
            roots,
            vec![
                PathBuf::from("/tmp/home/.config/tokscope/headless"),
                PathBuf::from("/tmp/home/Library/Application Support/tokscope/headless")
            ]
        );
...
",
  ...
  "projects": [
    ...
    // Windows cannot violate: the OS trims the name before tokscope sees a
    // directory at all.
    #[cfg(unix)]
