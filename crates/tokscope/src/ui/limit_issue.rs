...
use tokscope_core::{limits::LimitIssueKind, LimitSnapshot};
...
fn reauthentication_action(
    snapshot: &LimitSnapshot,
    profile_id: tokscope_core::accounts::CredentialProfileId,
    cx: &mut Context<TokscopeApp>,
    ...
    text_action(selector, "Sign in again", cx)
        ...
        .on_click(cx.listener(move |app, _, _, cx| {
            match tokscope_core::accounts::begin_reauthentication(provider, &profile_id) {
                Ok(()) => {
