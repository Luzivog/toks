...
use super::{
    AccountAvailability, BlockWindow, FastLimitDisposition, FastLimitOutcome, RotationRuntime,
...
fn a_usage_block_stops_only_the_thread_that_received_it() {
    ...
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.thread_attached(&account, &existing);
    runtime.thread_attached(&account, &sibling);
    runtime.replace_quota_drain(
        &BTreeMap::from([(account.clone(), Some(UnixMillis::new(100)))]),
        UnixMillis::new(10),
    );
...
fn a_fast_limit_falls_back_only_that_thread_until_the_fast_tier_resets() {
    ...
    let mut runtime = RotationRuntime::default();
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.thread_attached(&account, &victim);
    runtime.thread_attached(&account, &sibling);
    runtime.replace_quota_drain(
        &BTreeMap::from([(account.clone(), Some(UnixMillis::new(100)))]),
        UnixMillis::new(10),
...
fn an_account_admission_block_keeps_existing_threads_attached() {
    ...
    let mut runtime = RotationRuntime::default();
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.thread_attached(&account, &first);
...
fn drain_expiry_clears_thread_tier_and_block_overrides() {
    ...
    let mut runtime = RotationRuntime::default();
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.thread_attached(&account, &fast_limited);
    runtime.thread_attached(&account, &blocked);
    runtime.replace_quota_drain(
        &BTreeMap::from([(account.clone(), Some(UnixMillis::new(100)))]),
        UnixMillis::new(10),
    );
...
fn router_restart_preserves_a_task_awaiting_follow_up() {
    ...
    let mut runtime = RotationRuntime::default();
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.connection_opened(&account, &thread, UnixMillis::new(10));
...
fn a_pending_duplicate_reservation_survives_the_first_stream_closing() {
    ...
    let mut runtime = RotationRuntime::default();
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.reserve_thread(&account, &thread, UnixMillis::new(10));
    runtime.connection_opened(&account, &thread, UnixMillis::new(11));
    runtime.reserve_thread(&account, &thread, UnixMillis::new(12));
...
fn an_abandoned_selection_reservation_expires() {
    ...
    let mut runtime = RotationRuntime::default();
    runtime.reconcile(std::slice::from_ref(&account), UnixMillis::new(0));
    runtime.reserve_thread(&account, &thread, UnixMillis::new(1));
    runtime.replace_quota_drain(
        &BTreeMap::from([(account.clone(), Some(UnixMillis::new(1_000_000)))]),
        UnixMillis::new(2),
    );
